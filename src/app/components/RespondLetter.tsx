...
async function reportLetter(formData: FormData) {
  ...
  const report = await prisma.report.create({
    data: {
      ...
      reportContentAuthorId: String(letter.letterAuthorId),
    },
  });
...
export default async function RespondLetter() {
    ...
    return (
      <div className={container({ maxW: "4xl" })}>
        <p className={css({ fontSize: "3xl", textAlign: "center", p: "8" })}>
          To prevent spam, you can only respond to 5 letters an hour! Please try
          again later.
        </p>
        <Link href="/">
          <div
  ...
  if (hasLetter) {
    ...
    // if letter is null, letter has been deleted, reset letter to respond and reload to get new one.
    if (randomLetter === null) {
      await prisma.user.update({
        where: { id: sessionUser?.id },
        data: { letterToRespond: null },
      });
      redirect("/respondletter");
    }
  } else {
    ...
    if (unrespondedLetters.length !== 0) {
      ...
      // assign responseAuthorId in letter
      await prisma.letter.update({
        where: { id: Number(randomLetter.id) },
        data: { responseAuthorId: sessionUser.id },
      });
  ...
  async function submitResponse(formData: FormData) {
    ...
    // get letter
    const letter = await prisma.letter.findUnique({
      where: { id: randomLetter?.id },
    });

    // check if letter still exists
    if (letter === null) {
      // remove letterToRespond from user, redirect to fail
      await prisma.user.update({
        where: { id: String(session.user?.id) },
        data: { letterToRespond: null },
      });
    ...
    const letterAuthor = await prisma.user.findUnique({
      where: { id: letter.letterAuthorId },
    });

    // if letterAuthor is null, user has been deleted, redirect success anyway
    if (letterAuthor === null) {
      redirect("/success");
    }

    // if author is anon user, don't send email, redirect success
    if (letterAuthor.email.endsWith("example.com")) {
      redirect("/success");
    }
  ...
  return (
    <div className={container({ maxW: "8xl", pb:16 })}>
      ...
      {countryCode !== "" && randomLetter.letterCountry !== null ? (
        <div className={stack({ direction: "row" })}>
          <div className={center()}>
            Written by a stranger from: {randomLetter.letterCountry}
          </div>
          <Image
            src={countryUrl}
            alt={randomLetter.letterCountry}
            width={32}
            height={32}
            className={css({
              filter: {
